...
#### 컴포넌트
  ...
  1. 각 컨텐츠 영역마다 컴포넌트화 하여 사용(재사용) *** 
  2. 컴포넌트예시 : 헤더바, 배너, 리스트 등등, 조회조건, 각 중복반복되는 화면등.
  3. 조건 : 함수로 구성, UI 요소 return
  4. ```부모컴포넌트```, ```자식컴포넌트``` : 컴포넌트 안에 컴포넌트가 있을때, 부모, 자식 컴포넌트라 부른다.
...
#### JSX 문법
  ...
  3.  ```{}``` 을 사용하여 사용.
  4.  관련 URL : [https://velopert.com/867](https://velopert.com/867)
  5.  사용예시
