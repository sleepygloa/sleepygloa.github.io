...
jekyll 블로그는 ```Ruby```, ```liquid``` 로 작성되어있으나 필요한 부분 말고는 알필요없고, 많은 이용자들이 ```Template``` 을 제공해주고 있기 때문에 쉽게 블로그를 만들수 있다.

최소한의 블로그를 만들기 위해서는 ```Github``` 과 ```jekyll```, ```markdown``` 을 알아야하며, 원하는 블로그로 꾸미기 위해서는 ```Ruby```, ```liquid```, ```html```, ```css```을 알고 있으면 원하는 블로그를 쉽게 만들 수 있다.
