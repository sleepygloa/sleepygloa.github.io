...
- ```2020.09 - 2020.10``` :  TMS(Transit Management System) 개발
  - WMS 시스템과 연계된 고정배차 시스템
  - ```Daum API``` 를 이용한 실시간 배차 조회 및 월정산 시스템 개발
- ```2019.08 - 2020.06``` : TMS 개발
  - ```Tmap API```, ```Opinet API```와 다양한 마스터(노선, 차량, 운전자)를 이용한 최단거리 고정배차스시템 개발.
  - PL 로서의 역할(고객사와 업무 협의 및 배송기사앱(납품관련), 다양한 마스터를 이용한 정산시스템 도움)
- ```2018.06 - 2019.05``` : WMS 개발
  - 호치민, 하노이 출장.
  - JQuery 를 이용한 웹앱 및 반응형 화면 개발.
- ```2018.02 - 2018.05``` : WMS 개발
  - JQuery 를 이용한 간단한 화면 개발
  - Android 소스 버전관리 프로그램 개발, 세션 중복생성 버그 수정.
- ```2017.11 - 2018.01``` : MMS(Manager Management System) 개발 
  - JQuery 를 이용한 간단한 화면 개발(자산관리, 예방정검)
